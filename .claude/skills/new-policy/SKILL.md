...
1. **Gather requirements** — If the user did not supply a description, ask:
   ...
   - Destination file path? (default: `crasp.policy.yml` in the project root, or a named path if the user wants a non-default policy)
...
- If the policy is meant to replace `crasp.policy.yml`, remind the user that Crasp auto-loads that file from the working directory if no `--policy` flag is given.
