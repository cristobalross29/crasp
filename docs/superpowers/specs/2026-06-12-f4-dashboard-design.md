...
The renderer imports the existing presentational helpers from
`src/cli/commands/hook-log.ts` so the two views stay visually consistent
(`icon()`, `outcomeLabel()`, time formatting). To import them, those helpers are
promoted from file-private to **exported** (an additive, non-behavioural change —
see "Files to change"). Nothing about `hook-log`'s output changes.
...
- Deterministic in `(entries, opts)`. Same inputs → byte-identical output. No
  `Date.now()`, no `process.*`, no filesystem.
- Shows **the last N events that fit**, where N is derived from `opts.rows` minus
  the header (3 lines) and footer (3 lines) chrome. Because `entries` is
  oldest→newest, the renderer slices the tail: `entries.slice(-capacity)` and
  prints them newest-at-bottom (so new events appear at the bottom edge, like a
  log tail). If `rows` is tiny, capacity floors at 1.
- Truncates long lines to `opts.cols` so a frame never wraps and corrupts the
  layout.
...
tick, compare `(size, mtimeMs)` to the last seen pair; if changed, re-read via
`readHookLog(root)` and re-render. A trailing debounce coalesces a burst of
appends (Claude firing many hooks in a second) into at most one re-render per
`pollMs` window, so the screen never thrashes.
...
3. Leaves the alternate screen (`\x1b[?1049l`) — this restores the user's prior
   terminal contents, so the dashboard leaves no scrollback litter.
4. Exits 0.

`cleanup()` is guarded by a `cleaned` flag so multiple signals can't double-run
it. The alt-screen + cursor restores are written even on an unexpected `throw` via
the `process.on("exit")` hook, so a crash never leaves the terminal in a hidden-
cursor / alt-screen state.
...
shell, not the renderer): a relative duration (`30m`, `2h`, `1d`) or an ISO
timestamp. The shell converts it to a `Date` and passes it to
`readHookLog(root, { since })`. With `--once`, this yields a clean
"what happened in this session" snapshot. Without `--once`, the live view simply
starts from that floor and grows. Parsing lives in a tiny pure helper
`parseSince(spec, now): Date | undefined` so it is unit-testable independently of
the loop.
...
- `src/cli/watch-render.ts` — **new** — pure `renderDashboard()`, `tally()`,
  `parseSince()`, and the layout helpers.
- `src/cli/commands/watch.ts` — **new** — thin IO shell `watchCommand()`: TTY vs
  no-TTY branch, the debounced poll loop, alt-screen + teardown.
- `src/cli/commands/hook-log.ts` — **modify (additive only)** — `export` the
  presentational helpers `icon`, `outcomeLabel`, `fileDisplay`, `commandDisplay`,
  and the time/day formatters so the renderer reuses them. No output change.
...
- `tests/cli/watch-render.test.ts` — **new** — unit tests of the pure renderer,
  `tally`, `parseSince` (the bulk of coverage).
- `tests/cli/watch.test.ts` — **new** — CLI integration: no-TTY snapshot, `--once`,
  `--since`, empty-log, missing-`.crasp/`, exit code.
...
2. **The IO shell is deliberately thin and tested only at its observable edges.**
   CLI integration tests spawn `node dist/index.js watch …` and assert on a
   **terminating** invocation only:
   ...
   - `--since 0s` vs `--since 1d` change which seeded entries appear.

   We **never** spawn the infinite live loop in a test and try to "catch" a frame
   — that is the flaky path and we avoid it entirely. The live loop's only logic
   beyond the (already-tested) renderer is "poll → on change, re-render", which is
   thin glue; its correctness is argued structurally, not asserted by racing a
   subprocess. (An optional, non-default test could drive the poll tick via an
   injected timer, but the plan does not require it — see open questions.)
...
- **Driving the poll tick in a unit test.** We could inject a clock/timer into the
  loop to unit-test "append → re-render" without a subprocess. Decision: **not in
  v1's required tasks** — the renderer holds all logic and is fully tested; the
  glue is thin. Left as an optional hardening fast-follow so we don't over-build
  test scaffolding for trivial glue.
- **`--since` duration grammar.** v1 supports `Ns/Nm/Nh/Nd` and ISO-8601. Cron-ish
  or natural-language specs are out of scope.
